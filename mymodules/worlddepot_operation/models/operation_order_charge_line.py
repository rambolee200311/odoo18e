...
class OperationOrderChargeLine(models.Model):
    _name = "operation.order.charge.line"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = "Handover Charge Line (AR)"
    _order = "id desc"

    handover_id = fields.Many2one("operation.order.handover", string="Handover", required=True, ondelete="cascade", index=True)
    charge_type = fields.Selection([("quotation", "Quotation"), ("manual", "Manual")], string="Charge Source", default="manual", required=True)

    project_id = fields.Many2one("res.partner", string="Project/Customer",compute="compute_project_currency", store=True, readonly=True)
    currency_id = fields.Many2one("res.currency", string="Currency", compute="compute_project_currency", store=True, readonly=True)
