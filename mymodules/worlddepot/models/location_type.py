...
class StockLocationType(models.Model):
    ...
    def check_code_name_unique(self):
        for record in self:
            if record.code:
                existing_code = self.sudo().search([('code', '=', record.code), ('id', '!=', record.id)])
                if existing_code:
                    raise ValidationError(_('Location Type code "%s" must be unique.') % record.code)
            if record.name:
                existing_name = self.sudo().search([('name', '=', record.name), ('id', '!=', record.id)])
                if existing_name:
