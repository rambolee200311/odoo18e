...
class InboundProductImportWizard(models.TransientModel):
    ...
    def action_import_excel(self):
        for rec in self:
            ...
            if not inbound_order.reference:
                raise UserError(_("Inbound order reference is required before importing products."))
            if not rec.file:
            ...
            required_headers = ["reference", "pallet_no", "product", "product_ean", "quantity"]
            ...
            for row_number, row_data in rows:
                ...
                if reference != inbound_order.reference:
                    raise UserError(_("Row %s: reference must be %s.") % (row_number, inbound_order.reference))
                if not pallet_no:
