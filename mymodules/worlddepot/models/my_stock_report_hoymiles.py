...
class MyStockReportHoymiles(models.Model):
    _name = 'world.depot.store.report.hoymiles'
    _description = 'Stock Report for Hoymiles'
    ...
    products_ids = fields.One2many('world.depot.store.report.hoymiles.products', 'report_id', string='Products')
    ...
    def _get_products_data(self):
        ...
        if create_lines:
            ...
            self.env['world.depot.store.report.hoymiles.products'].create(create_lines)
    ...
    def action_get_products_data(self):
        ...
        return {
            ...
            'params': {
                ...
                'message': _(
                    'Inventory report has been generated successfully. Please refresh the page to see the updated data.'),
                'sticky': False,
                'type': 'success',
                'next': {
    ...
    def action_open_products(self):
        ...
        action = {
            ...
            'type': 'ir.actions.act_window',
            'res_model': 'world.depot.store.report.hoymiles.products',
            'view_mode': 'list',
...
class MyStockReportHoymilesProducts(models.Model):
    ...
    _description = 'Stock Report Products for Hoymiles'

    report_id = fields.Many2one('world.depot.store.report.hoymiles', string='Report Reference', required=True,
                                ondelete='cascade')
    container_no = fields.Char(string='Container No')
